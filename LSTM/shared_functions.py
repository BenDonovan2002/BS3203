...
try:
    # Validate constants
    assert isinstance(LEN_SEQ, int), "Sequence length must be an integer."
    ...
    text_chars = sorted(set(corpus))

    # Create a dictionary mapping each unique character to an index
    chars_to_indices = dict((c, i) for i, c in enumerate(text_chars))

    # Create a dictionary mapping each index to a unique character
    indices_to_chars = dict((i, c) for i, c in enumerate(text_chars))
